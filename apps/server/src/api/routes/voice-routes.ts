...
import type { VoiceCameraShare, VoiceCameraShareSignal, VoiceScreenShare, VoiceScreenShareSignal } from '@current/types';
import { z } from 'zod';
...
const ShareParamsSchema = z.object({ shareId: z.string().min(1) });
...
const MediaShareSignalSchema = z.discriminatedUnion('type', [
...
const MediaShareSignalBodySchema = z.object({
  sessionId: z.string().min(1),
  targetUserId: z.string().min(1),
  signal: MediaShareSignalSchema,
});
...
function broadcastStoppedCameraShares(app: FastifyInstance, shares: VoiceCameraShare[]): void {
  for (const share of shares) {
...
export async function registerVoiceRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/voice/channels/:channelId/join', { preHandler: [requireAuth] }, async (request, reply) => {
    ...
    try {
      ...
      const stoppedCameraShares = app.appContext.cameraShare.stopUserShares(request.currentUser.id);
      broadcastStoppedScreenShares(app, stoppedShares);
      broadcastStoppedCameraShares(app, stoppedCameraShares);
      ...
      reply.send({
        ...
        camera: app.appContext.cameraShare.getClientSettings(),
        cameraShares: app.appContext.cameraShare.listChannelShares(params.data.channelId),
      });
    } catch (error) {
  ...
  app.post('/voice/screen-shares/:shareId/signal', { preHandler: [requireAuth] }, async (request, reply) => {
    const params = ShareParamsSchema.safeParse(request.params);
    const body = MediaShareSignalBodySchema.safeParse(request.body);
    if (!params.success || !body.success || !request.currentUser) {
      reply.code(400).send({ error: 'Invalid request.' });
      return;
    }
  ...
  app.get('/voice/channels/:channelId/camera-shares', { preHandler: [requireAuth] }, async (request, reply) => {
    const params = ChannelParamsSchema.safeParse(request.params);
    if (!params.success) {
      reply.code(400).send({ error: 'Invalid request.' });
      return;
    ...
    reply.send({
      settings: app.appContext.cameraShare.getClientSettings(),
      shares: app.appContext.cameraShare.listChannelShares(params.data.channelId),
    });
  });

  app.post('/voice/channels/:channelId/camera-shares', { preHandler: [requireAuth] }, async (request, reply) => {
    const params = ChannelParamsSchema.safeParse(request.params);
    const body = SessionBodySchema.safeParse(request.body);
    if (!params.success || !body.success || !request.currentUser) {
      reply.code(400).send({ error: 'Invalid request.' });
    ...
    try {
      const { share, stoppedShares } = app.appContext.cameraShare.startShare({
        userId: request.currentUser.id,
        channelId: params.data.channelId,
      });
      broadcastStoppedCameraShares(app, stoppedShares);
      app.appContext.gateway.broadcastEphemeral(GatewayEvents.VOICE_CAMERA_SHARE_STARTED, {
        cameraShare: share,
      });
      reply.send({
        share,
  ...
  app.post('/voice/camera-shares/:shareId/stop', { preHandler: [requireAuth] }, async (request, reply) => {
    const params = ShareParamsSchema.safeParse(request.params);
    const body = SessionBodySchema.safeParse(request.body);
    if (!params.success || !body.success || !request.currentUser) {
      reply.code(400).send({ error: 'Invalid request.' });
    ...
    try {
      const share = app.appContext.cameraShare.stopShare({
        shareId: params.data.shareId,
        userId: request.currentUser.id,
      });
      if (!share) {
        reply.code(404).send({ error: 'Camera share not found.' });
        return;
      }
      broadcastStoppedCameraShares(app, [share]);
      reply.code(204).send();
    } catch (error) {
      sendVoiceError(reply, error);
    }
  ...
  app.post('/voice/camera-shares/:shareId/signal', { preHandler: [requireAuth] }, async (request, reply) => {
    const params = ShareParamsSchema.safeParse(request.params);
    const body = MediaShareSignalBodySchema.safeParse(request.body);
    if (!params.success || !body.success || !request.currentUser) {
      reply.code(400).send({ error: 'Invalid request.' });
    ...
    const share = app.appContext.cameraShare.getShare(params.data.shareId);
    const fromUserId = request.currentUser.id;
    if (!share) {
      reply.code(404).send({ error: 'Camera share not found.' });
      return;
    ...
    if (share.userId !== fromUserId && share.userId !== body.data.targetUserId) {
      reply.code(403).send({ error: 'Camera share signaling must be between the sharer and a viewer.' });
      return;
    }

    app.appContext.voice.touchSession(body.data.sessionId);
    app.appContext.gateway.sendEphemeralToUser(body.data.targetUserId, GatewayEvents.VOICE_CAMERA_SHARE_SIGNAL, {
      channelId: share.channelId,
      shareId: share.id,
      fromUserId,
      targetUserId: body.data.targetUserId,
      signal: body.data.signal as VoiceCameraShareSignal,
    });
    reply.code(204).send();
  });
